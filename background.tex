...
\glsentrydesc{FDR}~(\gls{FDR}) $[$\citet*{Benjamini1995-nf}$]$ is (in \ms\ proteomics)
...
Furthermore, this approach can guide
...
The key elements of this method are the creation of \emph{decoy} sequences
that are incorrect but similar to the \emph{target} (\ie\ true) ones,
and that any \psm\ found for a decoy sequence is by definition spurious.
...
where: \\{\small
$\text{\emph{Number of accepted \glspl{PSM}}}_{\text{decoy}}$ is the number of \glspl{PSM}
from decoy sequences that are above a specified threshold $\mathcal{t}$,\\
$\text{\emph{Number of all accepted \glspl{PSM}}}$ is the total number of accepted \glspl{PSM}
above the specified threshold $\mathcal{t}$,
including the accepted decoy \glspl{PSM}.\\
$\text{\emph{Number of accepted \glspl{PSM}}}_{\text{target}}$ is the number of accepted \glspl{PSM}
above the specified threshold $\mathcal{t}$.
}

The search is performed with the same parameters for the target peptide sequence database
and the decoy one.
To be the most effective,
the decoy sequences have to be as similar as possible to the target ones
(in terms of \gls{aa} composition, length, mass, charges, search assigned scores).
There are different ways to design the decoy sequences
(\eg\ reversed, shuffled,\ldots).
While the search can be done independently on the target and the decoy database,
usually they are both concatenated and the search is realised at once.
