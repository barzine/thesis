...
They contain the instructions for making the proteins,
...
to create a corresponding \emph{messenger} \gls{RNA} (\gls{mRNA})
from free \gls{RNA} nucleotides~\mycite{molBiolCell}.
The transcription is a directional process and
always happens from the 5' end of the gene towards its 3' end~\mycite{Callen2005-ik}.
...
and through time.~\mycite{molBiolCell}
...
the \mRNAs\ have have to be exported
...
from the free amino acids carried by \glspl{tRNA}
...
\glspl{PTM} allow the regulation of the proteins' activity (activation and deactivation).
...
or glycosylation of the amino acids (also called residues).~\mycite{molBiolCell,Morris2016-qv}
...
that can constitute the proteins (\Cref{sec:aa}).
Subsequently,
the latter also present an extensive range of
physicochemical properties.~\mycite{Morris2016-qv,Callen2005-ik}

Although the diversity of the proteome
(\ie\ entire repertoire of expressed proteins)
is the primary contributor to
the ending phenotype and functions of cells and tissues,
...
from the phosphate group that is linked to the pentose carbon (C) annotated 5'
to the phosphate group that is linked to the pentose carbon (C) annotated 3'.~\mycite{Morris2016-qv,molBiolCell,Callen2005-ik}

\vspace{-2mm}
\begin{figure}[!htpb]
    \includegraphics[scale=0.48]{background/DNA-RNAstruct1.png}\centering
    \vspace{-2mm}
...
Besides,
in eukaryotic cells,
while the genome is protected in organelles such as the nucleus,
...
Since its formulation by Crick \mycite{Crick:1958,Crick:1970},
the core of the central dogma of molecular biology, shown in \Cref{fig:dogma}
still holds true:
...
into the \gls{DNA},
we will be able to predict the phenotype due to the proteome.
As \gls{DNA} is static, genome studies are easier
than transcriptomic or proteomic ones.
However,
depending on the tissue or cell type,
the coding portion (about 2\%) of the human genome~\mycite{Venter2001-sn}
varies in its expression (both in concentration and composition);
there are ubiquitous proteins
(generally qualified as \emph{housekeeping}),
cell/tissue/condition specific proteins and
everything in-between.

\begin{figure}[!htbp]
    \includegraphics[scale=0.60]{background/dogma1.pdf}\centering
...
and as such its study has emerged as a reasonable trade-off strategy to study phenotypes.
...
three proteomic datasets used in this thesis are the product of this specific
...
protein/peptide ionisation and fragmentation methods)~\mycite{Aebersold2003,%
...
and their molecular mass is only recorded. However, it is quite common for the
...
mentioned previously, is very widespread. The chosen ion is introduced in the
...
and so they may also be unfitted for analyses.
...
\begin{itemize}
    ...
    \item A \latin{de novo} \emph{sequencing} approach which consists of
        comparing the observed mass data to
        the theoretical mass of every possible peptide sequence.
\end{itemize}
...
Hence, those are unfitted for other types of studies (such as this thesis).
...
following three hierarchically structured and controlled vocabularies (ontologies):
...
For example, if for \gls{go} term \textsc{A} is associated to 0.1\%
of all the background (list) genes,
but then over 70\% of the genes from the selected list are associated
with \gls{go} term \textsc{A},
one can safely accept that this selected genes list is enriched for this term.
...
Batch effects are due to all the variables that the investigator can not control
(either by lack of technology, design or knowledge), for example
environmental conditions, reagent or sample lots,
genetic population background.
Batch effects are often the source of complication for many studies,
including the high-throughput genomic studies~\mycite{batchEffect}.

The danger lies in overlooking them, and then confound these artefacts
...
It allows to test for the variability of the protocol itself.
While, using the same sample to test different protocols
may also literary referred as \enquote{technical},
...
They allow to assess the biological variability
