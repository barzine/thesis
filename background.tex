...
\section[Proteome exploration with Mass spectrometry]{Proteome~exploration~with~Mass~spectrometry}\label{sec:exploreProtMS}
...
\footnote{\cpv{See \cref{subsub:enzdigest}}}
...
through \gls{DDA} bottom-up label free approaches (see \Cref{ch:datasets}),
in the following section,
...
these methods provide only relative protein quantification.\cpvD\mybr\
...
These steps help the separation of the protein complexes,\cpvD\
the relative linearisation of the proteins and, to some extent,
...
\subsubsection{Proteolytic digestion}\label{subsub:enzdigest}
...
Although there are other enzymes
for restrictive digestion~\mycite{Giansanti2016-jf,Tsiatsiani2015-sf},
trypsin is the gold standard protease~\mycite{Zhang2014}.
\cpv{Trypsin is a serine protease that
has a high proteolytic and highly specific activity:
it cleaves the proteins} at the carboxyl side of an arginine (R) or lysine (K) \glspl{aa},
when they are not followed by a proline (P).
\cpv{Trypsin's cheapness and ease of use can also explain its popularity.}
Besides, trypsin creates peptides that are in the ideal range for \ms\ studies
as it produces a large number of short ($600$ to $1,000$ \gls{Da}) peptides that
can be efficiently fragmented and identified,
although inferring the parent proteins remains challenging~\mycite{Laskay2013-xv}.
Circularity may also contribute to this trend:
as more studies are using it, more data are available for comparison;
hence more studies employ it.\mybr\

Peptides produced by trypsin digestion are referred to as \gls{tryptic} peptides.\mybr\
\subsubsection{Separation methods (fractioning)}\label{subsub:sepMethods}
Fractioning (or fractionation) is the principal method
to simplify sample complexity.
It also allows focusing selectively on subcellular fractions when needed.
Indeed, one may want to study particular organelles,
cell compartments or other kinds of the proteome
(\eg\ phosphorylated or glycosylated proteins)~\mycite{Cox2011}.
Besides, fractioning is also a good strategy
to reduce the impact of undersampling and increase repeatability between analyses.
With \gls{MS} being prone to undersampling,
repeated analyses may fail to yield the same protein identifications,
...
ratio.
The 1D protocol is faster than the 2D one, while the latter is more selective
...
\subsection{Characterisation through fragmentation profiles}\label{subsec:msProfiles}
\vspace{-3mm}
...
\gls{MALDI} is very useful for large molecules.
It allows creating ions with a minimum of fragmentation (if any at all).
The molecules are fixed onto a matrix (a gel)
and then a pulsed \gls{laser} irradiates the sample which provokes the
ablation and then the desorption from the matrix and ionisation of the molecules.\cpvD\
...
\subsubsection{Fragmentation techniques~\mycite{Zhang2014}}\label{subsec:fragmentaTech}
...
\softFoCi{OpenMS}{http://www.openms.de/}{Pfeuffer2017-dh},
\cpv{\softFoCi{Skyline}{https://skyline.ms/}{Pino2020-dl} or
\softFoCi{Crux}{https://crux.ms/}{Park2008-ry,McIlwain2014-dd}.}
...
\cpv{\softFoCi{Spectronaut}{https://biognosys.com/spectronaut}{Bernhardt2014-gw}
is one example of specialised pipelines.
\soft{Spectronaut} handles the targeted analysis of \gls{DIA}\footnote{%
See \Cref{subsub:msAcquisitionMode}.} data.}
...
\cpv{spectra from a first mass spectrometer (MS1) allow
the selection of the ionised peptides (\ie\ precursor ions) to be fragmented,
while the spectra from the second one (MS2) allow
their identification~\mycite{Wang2019-wo,Codrea2016}.
Identification robustness increases
with the number of spectra associated with each peptide (or protein).}\mybr\

\minisec{Matching spectra to peptide sequences}

\cpv{In bottom-up proteomics experiments,
peptides can produce various fragments
through the cleavage of covalent bonds or internal rearrangements
(\eg\ loss of carbonyl group, a water or ammonium molecule)~\mycite{Macias2020-iu,%
Wysocki2005-ip,Yague2003-np,Bythell2010-dx}.}\mybr\

\cpv{While different protocols and parameters can produce similar fragments,
the peptide \gls{MS/MS} profiles (spectra)
recorded by a given instrument and fragmentation mode are unique.
Only fragments carrying at least one charge can be detected and measured.
The product ions' nature and relative abundances depend on
the initial peptide sequence,
and the chosen fragmentation method\footnote{See \Cref{subsec:fragmentaTech}.}
and energy\mycite{Revesz2021-ei}.
As no combination can reproduce any other one,
studies can benefit from using several ones~\mycite{Revesz2021-ei,%
Dupree2020-ps, Tu2016-er, Diedrich2013-pj}.}\mybr\

\cpv{Regardless of the other parameters,
collision-induced dissociations (\gls{CID} or \gls{HCD}) produce
many fragments that result from the peptide backbone fragmentation,
of which peptide bonds (see \Cref{sec:bio}) represent
the predominant breakage pathway
as they have the lowest energy \mycite{Dupree2020-ps, Medzihradszky2015-ta}.}\mybr\

\cpv{The Roepstorff–Fohlman–Biemann nomenclature~\mycite{Roepstorff1984-vc,%
Biemann1988-hj},
presented in \Cref{fig:nomRFBion},
has been widely accepted to designate the product ions generated
by the backbone fragmentation~\mycite{Medzihradszky2015-ta}.}\mybr\

\cpv{When a peptide bond (in purple in \Cref{fig:nomRFBion}) break occurs,
the precursor ion generates two complementary fragments.
The products are named as:
\begin{itemize}[topsep=0pt,nosep]
\item b-ions, for the fragments comprising the amino (or N-terminus) end of the precursor’s sequence,
\item y-ions, for the ones comprising the carboxyl (or C-terminus) end.
\end{itemize}
Before possible internal rearrangements\footnote{\eg, water loss
for fragments with Ser, Thr, Glu, or Asp~\mycite{Medzihradszky2015-ta}},
the sum of the mass of complementary ions equals
the molecular mass of their precursor ion.}\mybr\

\cpv{Other fragment types also exist,
and they assist with a better peptide characterisation\footnote{\ie\ peptide
identification}~\mycite{Noor2020-ig,Steen2004-qj,Wysocki2005-ip}.
For instance, one may use \gls{EThcD} to generate more informative fragments
to study peptides' phosphorylation or \gls{ADP}-ribosylation~\mycite{Penkert2017-zn,%
Penkert2019-mf,Bilan2017-tq}.
Furthermore, other protocols fragment the side chain (R group --- see \Cref{sec:aa})
and produce satellite ions (d-, v- and w-ions) that can help to differentiate
between isomers (\eg\ Leu and Ile)~\mycite{Han2007-mc,Johnson1987-rw}.
However, the \glspl{PTM}’ characterisation remains challenging~\mycite{Dupree2020-ps}.
}\mybr\

\cpv{The predictability of peptide fragmentation\footnote{Both
in the ions’ nature and their relative intensities~\mycite{Frank2009-nb}.%
}~\mycite{Dupree2020-ps,Medzihradszky2015-ta} and
the \glspl{aa} mass differences (see \Cref{tab:massAAs})
enable (at least partially) the peptide’s sequence resolution.
Overlaps of partial sequences obtained from different ion types
allow stitching them together in a fuller peptide sequence.}\mybr\

\cpv{\begin{figure}[!htbp]
    \includegraphics[scale=0.54]{background/nomenclature.png}\centering
    \vspace{-3.5mm}
    \caption[Roepstorff–Fohlman–Biemann nomenclature]{\label{fig:nomRFBion}\textbf{%
The Roepstorff–Fohlman–Biemann nomenclature}~\mycite{Roepstorff1984-vc,Biemann1988-hj}
unambiguously designates the different ions generated
from the peptide backbone fragmentation.
Here is an illustration for a peptide formed by eight \glspl{aa}
(one green box represents one \gls{aa} or its residue --- see \Cref{tab:massAAs}).
The most common breakage occurring by collision is
the cleavage of the peptide bond (amide bond, in purple),
which can produce a y-ion (when the C-terminus part of the precursor ion has a charge)
or a b-ion (the N-terminus part is charged).
The peptide backbone fragmentation can generate other series of ion types,
\eg\ \gls{ETD} and \gls{ECD} can produce c- and z-ions
(cleavage of the N-C$_\alpha$ bonds)~\mycite{Han2007-mc}.
Like b-ions, a- and c-ions are produced
when the N-terminal part of the peptide holds a charge,
while, like the y-ions, x- and z-ions are created
when the C-terminal part remains charged.
An index notation distinguishes the ions from the same series:
the index is the number of residues comprised (even partially) by the ions.
When considering a couple of complementary ions, \eg, b$_2$-ion and y$_6$-ion,
the indices sum equals to the residues number of the precursor ion;
in other terms, if $n$ is the number of \glspl{aa} in the peptide,
b$_m$-ion and y$_{(n-m)}$-ion are complementary.}
\end{figure}}

\cpv{\Cref{fig:MSspectraSeq} shows an \gls{MS/MS} spectrum,
also called fragmentation spectrum or MS2 spectrum.
When considering a series of either b- or y-ions,
the mass difference between two (single charged) consecutive ions corresponds to
the mass of the residue at the end\footnote{either at the N-terminal end
for a-,b- or c-ions or
at the C-terminal end for the x-, y- and z-ions}
of the lengthiest (hence the heaviest) ion of the pair.}\mybr\

\cpv{Although based on a real example,
\Cref{fig:MSspectraSeq} shows a simplified \gls{MS/MS} spectrum
to ease its interpretation.
In practice, assigning peaks is more challenging,
including assigning to one series over another one.
The lower the mass accuracy,
the more difficult it is to discriminate
between possible isobaric combinations and different ion types~\mycite{Medzihradszky2015-ta}.}\mybr\

\begin{figure}[!htb]
       \centering
       \captionsetup[subfigure]{justification=centering}
       \begin{subfigure}{1.1\textwidth}
           \includegraphics[scale=0.52]{background/MSMSspectra.png}\centering
           \vspace{-4mm}
           \caption{Simplified representation of an \gls{MS/MS} spectrum
           (or MS2 spectrum)\\for the peptide \textit{IYEVEGMR} ---
           adapted from \citet{Sadygov2004-nl}}\label{fig:MSMSspectra}
       \end{subfigure}

       \begin{subfigure}{1.1\textwidth}
           \includegraphics[scale=1]{background/seqMSMS.png}\centering
           \vspace{-4mm}
           \caption{Peptide sequence solved from the MS/MS spectrum.}\label{fig:seqMSMS}
       \end{subfigure}
       \caption[MS/MS spectrum and peptide identification.]{\label{fig:MSspectraSeq}\textbf{MS/MS
       spectrum and peptide identification.}
       In \subref{fig:MSMSspectra},
       two consecutive ions of the same series have a mass difference
       that corresponds to the mass of one \gls{aa} contained in one ion
       but missing in the other.
       The supplementary \gls{aa} is found at the C-terminus of the heaviest ion.
       The respective \textit{EVEGM} and \textit{YEVEG} partial sequences
       can directly be solved from the b- and y-ion series.
       By combining the \gls{MS/MS} spectrum presented in \subref{fig:MSMSspectra} and
       the precursor molecular mass given by the MS1 spectrum ($997.16$ \gls{Da}
       --- highlighted in red in \subref{fig:seqMSMS}),
       one can deduce the remaining peptide sequence.
       Mass ions directly extracted from \subref{fig:MSMSspectra} are
       in orange for the b-ions and in blue for y-ions.
       \Glspl{aa} with supporting evidence in \subref{fig:MSMSspectra}
       are highlighted in \subref{fig:seqMSMS} in orange when backed by b-ions
       and in blue by y-ions.
       The sum of the masses of complementary b- and y-ions equals the precursor's molecular mass.
       The complementary ion of y$_7$ has a mass of $114.16$ \gls{Da},
       which corresponds to Leu’s or Ile's acylium ion.
       Leu and Ile share the same mass and are undistinguishable
       from their b- or y-ions only.
       It may happen in the literature that a place holder
       is used to symbolise either of them,
       \eg\ \emph{J}, which refers to none of the standard \glspl{aa}.
       At the C-terminal end of the sequence,
       the complementary ion to b$_7$ has a mass of $175.20$ \gls{Da}
       that corresponds to Arg or
       the couple Val/Gly, an isobaric combination to Arg.
       However, the precursor peptide has been produced by trypsin digestion that
       specifically cleaves at the carboxyl side of Arg and Lys
       (when not followed by a Pro).
       Thus, it is more likely that the remaining sequence comprises Arg only.
       Theoretically, peptide sequences can be solved by
       collecting and identifying all the ions from the same series and
       correlating the mass differences between them
       with the residue masses of the amino acids
       --- including any possible \gls{PTM} or other modification.
       However, even when excluding Leu’s and Ile's ambiguity,
       a direct resolution is hard to achieve in practice
       as series overlap and ions may not be detected or recognised
       (\eg\ Ser and Glu can lose a water molecule
       through internal rearrangement \mycite{Medzihradszky2015-ta}).}
\end{figure}
\clearpage

\cpv{Besides, sufficient ions coverage is required,
but not all peptide bonds may break or be intense enough to be called.
In addition, water and ammonium losses and other modifications
can shift the mass of the residues.
However, there are also many thumb rules (see \citet{Steen2004-qj})
providing quick guidance about reasonable sequences.
Furthermore, possible iminium ions (\chemForm{+H_2N=CHR}) may help determine
the \glspl{aa} composition of the precursor peptide,
and past studies have highlighted many fragmentation rules
that contribute to removing ambiguities.
See \citet{Medzihradszky2015-ta} for a more detailed discussion on
fragmentation rules and \gls{MS/MS} spectra interpretation refinements.}\mybr\

\cpv{As modern \gls{LC-MS/MS} experiments can produce
tens of thousands of \gls{MS/MS} spectra~\mycite{OBryon2020-ne},
manual approaches are neglected in favour of
algorithms to automate the peptide spectra matching.}\mybr\

\cpv{There are three main possibilities for protein/peptide spectra matching:
the sequence-based searching approach,
the spectral library searching one and the de novo sequencing:}\mybr\
\vspace{-3mm}
\begin{itemize}[topsep=0pt,nosep]
    \item \cpv{The sequence based approach
        matches the experimental spectra to theoretical ones.
        These theoretical spectra} are created by
        ...
        \cpv{\softCi{Sequest}{Eng1994-eq,Tabb2015-uu},
        \softCi{Andromeda}{Cox2011-no} from \soft{MaxQuant},
        \softCi{X!Tandem}{MacLean2006-lh},
        \softCi{MS-GF+}{Kim2014-nj},
        \softCi{MS Amanda}{Dorfer2014-re},
        \softCi{Open-pFind}{Chi2018-rh},
        \softCi{MSFragger}{Kong2017-fb},
        \softCi{Morpheus}{Wenger2013-zf}
        or \soft{Pulsar} included in Spectronaut\textregistered{}.}
    \item The spectral library based approach
        ...
        \cpv{\softCi{SpectraST}{Lam2008-qy} or \softCi{BiblioSpec}{Frewen2007-jm} from \soft{Skyline},
        \softCi{M-Split}{Wang2010-nb}, \softCi{Pepitome}{Dasari2012-yy},
        \softCi{QuickMod}{Ahrne2011-uj},
        \softCi{pMatch}{Ye2010-of},
        \softCi{COSS}{Shiferaw2020-av} or
        \softCi{ANN-SoLo}{Bittremieux2018-ce}.}
    \item The \latin{de novo} \emph{sequencing} approach consists of
        comparing the observed mass data to the theoretical mass
        of every possible peptide sequence.
        \cpv{This method is the closest to the manual approach described above.}
        For example, see \softFoCi{PepNovo}{http://proteomics.ucsd.edu/Software/PepNovo/}{Frank2009-nb},
        \cpv{\softFoCi{PEAKS}{https://www.bioinfor.com/peaks-studio/}{Ma2003-pb},
        \softFoCi{pNovo $3$}{http://pfind.ict.ac.cn/software/pNovo/}{Yang2019-fe},
        \softCi{DeepNovo}{Tran2017-ct},
        \softFoCi{MetaSPS}{http://proteomics.ucsd.edu/software-tools/metasps/}{Guthals2013-ah},
        \softFoCi{Novor}{rapidnovor.com}{Ma2015-bz},
        \softCi{Lutefisk}{Taylor1997-xl},
        \softCi{NovoHMM}{Fischer2005-ym},
        \soft{ANTILOPE} from \soft{OpenMS},
        \softCi{Twister}{Vyatkina2015-gu} for topdown data,
        \softFoCi{UniNovo}{http://proteomics.ucsd.edu/Software/UniNovo}{Jeong2013-ss},
        \softFoCi{UVNovo}{https://github.com/marcottelab/UVnovo}{Robotham2016-rz}.}
\end{itemize}
...
\cpv{and requires high quality data for best performance~\mycite{Muth2018-pv}.}
Note that there are also hybrid approaches based on \latin{de novo} sequencing and
database matching,
\cpv{such as \softCi{GutenTag}{Tabb2003-lq},
\softCi{InsPecT}{Tanner2005-fn},
\softCi{DirecTag}{Tabb2008-ar},
\softCi{ByOnic}{Bern2012-hk},
or \softCi{PEAKS DB}{Zhang2012-ii}.}\mybr\

Many search engines exist for matching \gls{MS/MS} spectra,
\cpv{see \citet{Noor2020-ig,Chen2020-rm,Griss2016,Shteynberg2013,Eng2011}} for reviews.
Furthermore\label{seg:moreAlgoisbetter},
combining the results of several search algorithms when possible improves
\cpv{the final outcomes~\mycite{Noor2020-ig,Chen2020-rm,Sadygov2004-nl,Eng2011,Shteynberg2013,Griss2016}.}\mybr\
...
and for which it is in practice impossible to calculate an optimal solution.\mybr\
...
\cpv{(see the below example based on \soft{Maxquant})}.\mybr\
...
to build their models (see \Cref{fig:ProtInfClass}, p.~\pageref{fig:ProtInfClass}).
...
complicate the inference.\mybr\

Note that if an edge connects a peptide $i$ and a protein $j$,
the peptide $i$ is said to be covered by the protein $j$~\mycite{Huang2012-nr}.\mybr\
...
would it mean then that only \textit{protein 1} is expressed?\mybr\
...
\cpv{One possible approach for protein inference is a parsimonious approach,
as implemented for example by \softCi{Maxquant}{maxquant2016,Cox2008-ee}}.\mybr\

\cpv{After identifying all proteins covering a given peptide,
\soft{Maxquant} joins the proteins with the same set or subset of peptides
in the same protein group.
In other words,
if the peptides set $S_a$, defining a protein $P_a$,
is equal to or strictly included in
the peptides set $S_b$, defining the protein $P_b$,
then $P_a$ and $P_b$ are joined in the same group $G_1$.
Then, in each group,
the proteins are ordered by the decreasing number of peptides they cover.
Hence, the protein sequence at the top of the group
can explain all the group's peptides.}\mybr\

\cpv{\soft{Maxquant} refers to a peptide as
\enquote{unique} when found in only one group of proteins
in contrast to degenerated peptides shared between two distinct protein groups
that cannot be combined because their other peptides are unique to each group.
Shared peptides are called \enquote{razor} (referencing Occam's razor)
in the protein group with the highest number of peptides
since it is considered the simplest explanation.}\mybr\

\cpv{The user can choose which peptides are included in the quantification:
unique peptides only, both unique and razor peptides
(default option\footnote{Considered by the authors as the best compromise
between most accurate protein quantification and unequivocal peptide assignment~\mycite{Cox2008-ee}.})
or all the peptides (of each group).
The following steps enable results filtering based on the protein groups \glspl{PEP}
(a protein group \gls{PEP} equals the multiplication of its peptide \glspl{PEP}),
the spectra quality,
the unique peptides number and the \gls{FDR} threshold.}\mybr\

\cpv{Many tools implement other approaches.}\mybr\


\cpv{\softCi{DTASelect}{Tabb2002-wm} sorts peptides by their identified locus
(\ie\ gene or protein identifier) and then by sequences.
Next, \soft{DTASelect} filters the results based on various criteria,
including the spectra quality and user’s inputs,
to finally keep proteins supported by
enough different peptides or by at least one peptide identified several times.
The algorithm adopts an optimistic approach
instead of a parsimonious one
and only groups together proteins that have a strict identical sequence coverage.}\mybr\

\cpv{\softCi{ProteinProphet}{Nesvizhskii2003-ls}
(part of the \soft{Trans-Proteomics Pipeline})
is one of the most widely used methods in the literature~\mycite{Sikdar2016-lm}.
The tool computes in each sample
the presence probability of a protein
by combining the probabilities of its different identified peptides
through an iterative process.
An \gls{EM} algorithm derives
a mixture model of correct and incorrect peptide identifications
from the observed data\footnote{including the \gls{PSM} results,
their associated score or the properties of the peptide matched to the spectrum}.
The following steps can summarise the inference\footnote{Based
on \citet{Nesvizhskii2006-co, Serang2012-jl}}.
\soft{ProteinProphet} keeps the best spectrum matching any given assigned peptide.
It retrieves all proteins that cover any identified peptide.
The tool groups the peptides by proteins and
computes the protein’s presence probability based on the available peptide evidence.
It readjusts the peptides' negative and positive distributions
based on their sibling counts:
proteins that have more than one assigned peptide are rewarded,
while \enquote{one-hit wonders} are penalised.
\soft{ProteinProphet} apportions the degenerate peptides
across all their covering proteins
before using a parsimonious approach to readjust their distributions
through an \gls{EM} algorithm.
As a starting point,
the sum of all the peptides’ weights of one protein equals one.
Through successive iterations,
these weights are refined such that
proteins with a higher number of peptides are rewarded.
All redundant and indistinguishable protein entries are finally collapsed together.}\mybr\

\cpv{The model learns directly from the observed data,
which increases its robustness~\mycite{Huang2012-nr}.
However, one identified issue with this award/penalty system is that
it creates cases where proteins covering many low-scoring peptides can outrank
proteins covering a smaller set of higher-scoring peptides~\mycite{Serang2012-jl}.
As the size of the dataset to study grows, the problem worsens.
A complimentary tool in the \soft{Trans-Proteomics Pipeline} developed by the authors, \softCi{iProphet}{Shteynberg2011-gz},
addresses this issue by considering other information levels
and refining the computation of the posterior probabilities
and protein \gls{FDR} estimates.}\mybr\

\cpv{\softFoCi{Percolator}{http://percolator.ms}{Kall2007-qn,The2016-dq,Halloran2019-px}
(part of both \soft{The OpenMS proteomics pipeline (TOPP)} and
the \softFo{Crux suite}{http://crux.ms/})
is based on \glspl{SVM}.
It implements a generalised semi-supervised\footnote{\emph{semi}-supervised
since only decoy \glspl{PSM} are labelled as \enquote{incorrect}
while target \glspl{PSM} are an unlabelled mixture of
correct and incorrect \glspl{PSM}.~\mycite{Halloran2019-px}}
learning approach distinguishing
between target and decoy\footnote{shuffled or reverse peptide sequences} \glspl{PSM}
for any shotgun dataset.
First, a classifier is trained with a subset of the data,
\ie\ the algorithm knows the data labels.
Second, after ranking targets and decoys according to a selection of features,
the algorithm selects the target \glspl{PSM} with a 1\% \gls{FDR}
and then trains an \gls{SVM} to discriminate
between the kept targets and the full set of decoy \glspl{PSM},
and induces a new ranking.
These steps iterate until convergence of the ranking
(\ie\ the ranking remains the same from one iteration to the next).}\mybr\

\cpv{\softFoCi{Fido}{https://noble.gs.washington.edu/proj/fido/}{Serang2010-hy}
(implemented first as a standalone tool,
but now distributed as a part of \soft{Percolator})
is based on Bayesian inference (see \Cref{app:bayesInf}).
From a set of simple assumptions,
the authors have developed a Bayesian model.
This model estimates three parameters directly from the data:
the probability of a present protein to generate peptides,
the error probability of peptides to be detected from noise
and the proteins’ a priori probabilities to be present in a sample.
These parameters are respectively annotated \alpha, \beta\ and \gamma.
\soft{Fido} explicitly allows high ranking spurious \glspl{PSM}.
It uses their presence likelihood\footnote{By converting
the \glspl{PSM} prior probability estimates back into
discriminant score-based likelihoods when needed}
and rewards proteins which include strong independent supporting evidence
besides their degenerate peptides.
\soft{Fido} automatically apportions information from degenerate peptides
while ensuring that each protein presence likelihood is unrestricted to
their degenerate peptides.
Compared to the initial approach of \soft{ProteinProphet},
this method’s accuracy is resistant to the dataset size.
A related method to \soft{Fido} is \softCi{EPIPHANY}{Pfeuffer2020-nr}.}\mybr\

\cpv{Another inference tool from \soft{Percolator}’s authors is
\softCi{Barista}{Spivak2012-re},
which is also part of the \soft{Crux suite}.
\soft{Barista} combines the verification of the \glspl{PSM} and the protein inference
where \soft{Percolator} handles only the second task.
The authors advocate for a topdown approach to optimise
the protein inference problem
instead of subdividing the workflow into independent modules.
Their tool builds a tripartite graph (spectra-peptide-protein) based on
the results of a database search (target and decoy sequences)
and a protein database.
A learning algorithm infers the protein presence likelihood in each sample.
This algorithm involves a similar set of features to \soft{Percolator}.
\soft{Barista} iteratively refines the peptide identification ranks
to better discriminate between correct and incorrect identification.
In contrast to \soft{Percolator},
the \glspl{PSM} are not filtered in \soft{Barista} and
contribute to the results optimisations.
The authors themselves state that assessing
which approach is the best in practice can require more study~\mycite{McIlwain2014-dd}.}\mybr\

\cpv{Many other inference algorithms exist,
including \softCi{PIA}{Uszkoreit2015-ly},
\softCi{MIPGEM}{Gerster2010-hz},
\softCi{PAnalyzer}{Prieto2012-iv},
\softCi{DBParser}{Yang2004-vb}
and \softCi{PANORAMICS}{Feng2007-aj}.
The field is steadily improving and adapting in response to
the development of the measurement instruments and acquisition protocols
(\eg\ \softCi{IPF}{Rosenberger2017-ir} for \gls{DIA} data),
preparation protocols
(\eg\ multiple proteolytic digestions in parallel, see \mycite{Miller2019-od}),
increase of computing resources (for example
\soft{Percolator} optimisations~\mycite{Halloran2019-px}),
optimisation or implementation of new mathematical approaches and concepts
(\eg\ \softCi{DeepPep}{Kim2017-gi} based on
a deep-convolutional neural network framework or
\softCi{gpGrouper}{Saltzman2018-du} and
\softCi{MIPGEM}{Gerster2010-hz} that implement a gene centric approach).
Furthermore, the rise of metaproteomic, proteogenomic, metabolomic
and multiomic studies has brought
new perspectives and challenges~\mycite{Gonnelli2015-rl,Starr2018-ar,%
Rechenberger2019-wt,Menschaert2017-vd,Liebal2020-ls}.}\mybr\

\cpv{\citet{The2018-gq} have designed
an experiment simulating homologous proteins that
can evaluate most protein inference algorithms.
They observe better concordance between inferred proteins and the ground truth
when excluding degenerate peptides rather than adopting a parsimonious approach.
However, they report that results are different
when considering protein groups instead of individually.}\mybr\
...
\cpv{Over the last decade,
along with the instruments' improvement and
the increasing number of label-free bottom-up proteomics,
there have already been many developments in quantification methods and tools.
\citet{Blein-Nicolas2016-ty, Chen2016, Lindemann2017-vh}
review many of the currently available ones.
Note that
methods created for label-free relative quantification experiment designs
can be adapted for absolute ones~\mycite{Pappireddi2019-jf,Sinitcyn2018-ss,Chen2016}.
Many normalisation methods have also been reviewed by \citet{Valikangas2018-yj}.}\mybr\

\cpv{As mentioned above,
label-free quantification methods are either based
on the number of identified peptides or
MS2 (i.e. \gls{MS/MS}) spectra matched to a protein or
on the precursors' ion extraction current peak intensities (\ie\ \gls{XIC})
from the MS1 spectra.}\mybr\
...
Many other methods (\eg\ \softCi{APEX}{apex-tool}, \softCi{emPAI}{emPAI-tool}
or \softCi{MAI/PLGEM-STN/SC}{Lee2019-oy}) are derived from it.
\citet{Cozzolino2020-tk} have realised a comparative study between their method
and two other spectral counting ones.\mybr\
...
that make it unfit for direct
use for a proportional assessment of the complete protein set;
\gls{IBAQ} dramatically underestimates low-abundant proteins.
Instead, to improve the general quantification estimation,
the authors advocate a \emph{Top3} approach~\mycite{Silva-Top3},
...
\cpv{MS1-based quantifications have been reviewed by \citet{Wang2019-gk}.}\mybr\

Many normalisations
include the protein sequences length
(longer proteins are more likely to produce a greater number of sampled peptides)
and the sum of ion intensity or
...
largely overestimates the \gls{FDR} for large datasets.
To overcome this,
...
For each pair, the target's and the decoy's protein scores are compared,
...
(and \enquote{\emph{picked}} \gls{TDA} actually underestimates \gls{FDR} in that case).\mybr\

\cpv{Several benchmarking studies~\mycite{Valikangas2018-uw,Al_Shweiki2017-rd,%
Bubis2017-lu,Navarro2016-qg}
have been published recently that may help choose
between the many existing quantification methods and
the tools implementing them.}\mybr\
...
For examples, see \citet{Oytam2016-rb,Gagnon-Bartsch2012-dj,ruvseqComQN}.\mybr\
...
\mRNA\ and protein levels should share \cpv{strong correlates}.
