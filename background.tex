...
or skipped without understandability issues.
...
85 to 70\% of the total mass of these molecules are identical
through every organism, and
15 to  30\% of them are very specific to each species.
While the inorganic molecules (water and salts) and
the smaller organic molecules
(such as simple sugars, amino or organic acids, nitrogenous bases,
lipids or their precursors) composed the main number of molecules present in a cell,
most studies focus on the macromolecules,
\ie\ \gls{DNA}, \gls{RNA} or protein populations of the cell.~\mycite{Callen2005-ik}

\Cref{fig:dogma} is the reconstruction by \citet{Crick:1958} of
what was conceived circa 1958 and is now known as
the core of the central dogma of molecular biology.
It presents the relationship between these three types of macromolecules:
one gene will be transcribed from \DNA\ to one \mRNA,
which will be in turn translated in one protein.
This still holds true even though the truth is not as linear as first thought.

\begin{figure}%[!htbp]
    \includegraphics[scale=0.45]{background/dogma}\centering
    ...
    F. Crick circa 1958.} The solid lines present the proven modes of
    information transfers, the dashed ones are the ones that were postulated but
    have yet to be established.
    Nowadays, we know that the arrow from \gls{RNA} to \gls{DNA}
    should be a solid one, even though this happens on particular occasions.}
\end{figure}

All the genetic material of a living organism
(\ie all its coding and non-coding \gls{DNA})
constitutes its genome.
In most cells,
strands of \gls{DNA} are tightly packaged around proteins called histones
and they form superstructures known as chromosomes;
for example, the human genome is split into 23 pairs of chromosomes.
The coding sections of the \gls{DNA} are the genes,
which contain the instructions for making the proteins.
When a gene is switched on,
it triggers a process called transcription.
An \gls{RNA} polymerase attaches to the start of the gene,
and uses the \gls{DNA} strand as a template
to create a corresponding \gls{mRNA}.
Usually, a few modifications occur
before the \mRNA\ is in turn used as template to make the protein.


\mRNAs\ constitute one part of the transcriptome,
which is the total repertoire of transcripts (\ie\ \glspl{RNA}
molecules) expressed in a cell (or group of cells such as a tissue)
...
depending of the biological context and time.
...
While double-stranded \gls{DNA} (which is the most common form for \gls{DNA})
and proteins are very stable,
\glspl{RNA} have rather short life.

Physicochemical properties of the \glspl{DNA} and \glspl{RNA} are very similar
as the notable differences are the nature of the monosaccharide they include and
the methylated or demethylated form of one of the pyrimidine nucleobase.
Proteins are a lot more diverse, because the amino acids are a lot more diverse
(and 3D folding are more complex and such may exhibe one type of or another kind
of hydrophobic or hydrophily or amphiphily and charges). Additionally,
many post-translational modifications can occurs (addition of phophorylated groupment,
addition of saccharides or lipides or a combination of these).
...
Differential expression analyses (DEA) have led to the development of distinct
...
instruments and methods to the extensive and routine use of \ms\ as an analytical
...
specific proteoforms (see~\cite{Chapman2014})}, and more precisely, the
...
\gls{FFPE} samples \mycite{Steiner2014}.
...
standard protease~\mycite{Zhang2014}.
Circularity also explains its popularity:
...
pressure may be applied (\eg\ \glsxtrfull{HPLC} or \glsxtrfull{UPLC}).
...
Mass spectrometers raw data output for proteomics is directly uninterpretable
...
most notably, the data matching (\ms\ spectra to database content) and
protein naming~\mycite{Nilsson2010}. Indeed, the sheer amount of produced data
is in the terabyte (\gls{TB}) range which prohibits any manual handling and requires
...
however, they may lack recording a few critical points of the raw data and thus they
may also be unfit to analysis.

There are many different pipelines. See \citet{Vitek2009}.
...
It may look trivial, particularly as the resolution of the instruments significantly
...
\section{Possible downstream expression analyses}\label{sec:enrichmentAnalysis}
