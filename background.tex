...
as it is developing from the 1980s onwards~\mycite{BiomolBio}.
...
However, it requiers compatible mass spectrometers
...
and hence a great tools for global discovery study.
...
On the other hand,
peptide subsets are also undetectable with the current available technology
(they may hardly ionise or their signal is weak and recovered
by other more abundant or more ionisable peptides).
Thus, shotgun experiments are plagued by missing data~\mycite{Stead2008-in,Lazar2016-oe},
and the protein identification processus leads to three computational challenges:
...
It can exploits many scores and other specific data features
to automatically determine the best threshold
without overfitting to a particular collection of \psms.
Thus, comparing results between studies and laboratories are facilated.
...
\vspace{-0.7mm}
Algorithms infering proteins identity also use
...
training sets of target and decoy \psms\ to learn the paramaters
...
\begin{figure}[!htbp]
    \includegraphics[scale=0.6]{background/MSstatsVal.pdf}\centering
    \vspace{-3mm}
...
While \glspl{PEP} are more indicated for single verification such as
the presence validation of an explicit peptide in a particular sample,
for high-throughput proteomics that survey the overall proteome landscape,
q-values are usually favoured
as the whole collection of \psms\ is considered~\mycite{Kall2008-tf}.
...
the main challenging issue~\mycite{Nesvizhskii2005-wd,He2016-yi}.
...
and the results validation (both at peptide and protein levels);
particularly as the peptide identification quality influences the protein one.
...
as a bipartite graph (\Cref{fig:bipartite})~\mycite{Huang2012-nr}.
This task is very challenging due to
the existence of \enquote{degenerate peptides} and \enquote{one-hit wonders}.
Degenerate peptides are ambiguous peptides
that are shared by multiple protein sequence definitions.
When these proteins are homologous with similar biochemical functions,
some workflows create protein clusters to analyse them together.
Typically, these ambiguous peptides are discarded and
...
They require cautious handling regardless their peptide uniqueness status,
since if the peptide identification is a false positive (\ie\ an artefact),
then the protein is also a false positive.
Shorter proteins are usually harder to identify (and quantify),
...
their identification is more reliable and less computationally challenging.

Despite the lack of overlap between the set of confidently identified proteins~\mycite{Searle2008-dx},
combining several search engine algorithms has proven to yield better results
than a single search.
At worst, it improves the confidence of the identification
as more peptides per protein are characterised~\mycite{Huang2012-nr,Audain2017-jj}.
...
\begin{figure}[!htb]
    ...
    \textbf{Protein inference: the bipartite graph.} To infer proteins,
    ...
    To achieve the inference,
    peptide assembly algorithms can rely on
    the bipartite graph as sole input (blue solid box)
    or they can include other data types as well,
    \eg\ the score associated to each \gls{PSM} (dashed blue circle) or
    the raw spectra itself (teal solid box).
    }
\end{figure}

\citet{Huang2012-nr} review the general assumptions
and problems of inferring correct proteins through peptide assembly.
...
\citet{Huang2012-nr} also organise in several categorisation frameworks
many different algorithms tackling this key step.
\Cref{fig:ProtInfClass} presents the classification based on
the underlying algorithmic technique.
The original article presents different models of algorithm and many example tools.

As this thesis reuses available published data,
our choice for peptide assembly method is rather conservative:
the parsimonious model,
so to avoid inputting additional data that could be missing or
on which we lack enough context or perspective.

\begin{figure}[!htb]
    \includegraphics[scale=0.64]{background/ProtInfClass.pdf}\centering
    \caption[Peptide assembly models classification]{\label{fig:ProtInfClass}%
    \textbf{Peptide assembly models classification proposed by \citet{Huang2012-nr}.}
    }
\end{figure}

\subsubsection{Protein quantification (label free)}

There are a few different methods to quantify (and normalise) label-free proteomics.
\glsentrydesc{IBAQ} (\gls{IBAQ}) method is based on peak intensity.
...
they argue to use a variation on \gls{IBAQ}:
...
Top3 allows a better quantification of the proteome landscape in general,
even if it is less accurate for the shortest proteins and
saturates for the largest ones.

Common normalisations
(\eg\ \gls{NSAF}~\mycite{Zybailov2006-nk}, see \Cref{eq:NSAF})
...
for each protein.
Similar approaches to \Rnaseq\ are used to allow protein comparison
across different samples.

Alike to protein inference,
