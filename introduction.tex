...
\species{Homo sapiens} for example) \mycite{Tamara2013}.
...
\soft{HTSeq-count}\footnote{\soft{HTSeq-count} ---
...
I briefly summarise their main characteristic below in their paper publication
order.

\TK{Isabelle, je travaille encore sur le reste.}


\subminisec{Cufflinks}
\soft{Cufflinks} is part of set of tool developed under the \soft{Tuxedo} suite
which also includes \soft{TopHat2} and \soft{Bowtie}.


\subminisec{HTSeq-count}
This stand-alone script, provided with the \gls{Python} library \soft{HTSeq},
handles more straightforwardly the feature quantification.

It discards any multiread and counts how many \emph{unambiguous} reads in SAM/BAM
file are overlapping the features of interest described in a given gene
model\footnote{Gene models are distributed as annotation file (usually either as
GTF or GFF format) to a reference (genome or transcriptome).}.

For example, to assess gene expression, it counts reads that overlap unambiguously
with the exons.

Initially, this simpler approach was designed for \emph{differential gene analysis}.
However, more



\begin{comment}
its exons. These counts can then be used for gene-level differential
expression analyses using methods such as DESeq2 (Love
et al., 2014) or edgeR (Robinson et al., 2010). As the script
is designed specifically for differential expression analysis, only
