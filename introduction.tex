...
non coding RNA''} and 366 have both tags. The remaining 637 have various tags
(including \comp{microRNA profiling by high-throughput sequencing} as only tag
for 4 of them)--- Query date: 22 June 2017.}
...
their 3'end as to block any further elongation.
The product of this synthesis is called a \emph{read} and supports the base calling.
...
new nucleotide that each cluster has incorporated. The signal intensity is also
considered while the base call and as the calling happens its accuracy is also
measured and recorded. Finally, the fluorescent tags and the 3' caps are cleaved
...
Once the programmed read length is achieved (typically between 30 to 200),
...
end-user with text files. These files are usually distributed in \fastq\ format
\mycite{fastqFormat} which record for each cluster (read) a unique identifier,
a nucleotide sequence and, for each base of the sequence,
a \gls{Phred} quality score. Some optional information can also be provided (\eg\
the position of the read on the \gls{flow} (See \cref{sec:fastq_format} for
a random read example).


The \gls{Phred} quality score ($Q$) are set by the base calling program and are
defined as $Q = -10\log_{10}(P)$ with $P$ the probability of the base being
called wrongly (See also \cref{sec:PhredScore}).



In paired end experiments reads are typically split over two ordered files, one with the first end and the other with the second.
