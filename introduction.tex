...
\emph{bottom-up} approach\footnote{\emph{Bottom-up approach}:
\glsentrydesc{Bottom-up}.\\In fact, \gls{Top-down} approaches while appealing
are still very challenging both as experimental and computational perspectives
\mycite{AebersoldMann2016}.}
for identification and quantification studies\footnote{Bottom-up approaches are
...
specific proteoforms (see \mycite{Chapman2014})}, and more specifically, the
typical \emph{\enquote{shotgun}} proteomics approach that
relies on \gls{LC-MS/MS} \mycite{Cox2011}, \mycite{Zhang2013} that has produced
the three proteomic datasets used within this thesis. The lately increased
...
\mycite{Cox2011}, \mycite{Zhang2014}.

The typical high-throughput bottom-up (including \gls{LC-MS/MS}) workflow
aims to decrease the sample complexity to analyse (thus shortening the analysis
time) while increasing the depth of proteomic coverage. Indeed, the proteomic
field unfortunately lacks amplification methods such as \gls{PCR} for
transcriptomics. In addition, while the human proteins are based on 20 amino acids,
many of them share sequence homology. Consequently, to improve the number of
characterised proteins, the peptide mixtures are first separated in smaller
fractions before being subjected to \ms\ \mycite{Aebersold2003},
\mycite{Cox2011},\mycite{Zhang2013}.
...
\TK{Separation techniques}

Folding of proteins and complexes with number of subunits: increase the difficulty
it is not like transcriptomic where the molecule can be more easily linearised by
denaturation.
