...
--- ici intro par rapport à Asimov et le fait que l'ADN est un ``blueprint''
...
hereinafter the currently most popular ones.
...
experiment steps.} and, more importantly, the amount of the reads (or fragments)
that may be mapped to a reference\footnote{The quantification disregarding the
\emph{unmapped} reads (or fragments) so does the normalisation.}.


\begin{comment}
Since the first use of \Rnaseq\ is to explore the \gls{RNA} population of samples
and compare them together, an early method of normalisation


Sampling bias problems: no idea what is being fished and there is no way yet
to assess the absolute RNA abundance of a sample (also a lot of things are not
measured: for example volume/weight of biological samples processed,
number/size of the cells. It is OK because in most cases,
there is a define question and many issue can be solved by correct normalisation
of the data.

There are many normalisation methods around. One should consider what they
are doing before picking one. (judment)

Unfortunately, \Rnaseq\ lacks (still) to assess the absolute abundance of each
gene in a sample.

Longer the genes and more event to pick it.
\end{comment}

First def was: read per kilobase of exon model per million mapped reads.

Now it is read per kilobase of transcript per million mapped reads
and then Fragments per kilobase of transcript per million mapped reads.
