...
\begin{figure}
    \includegraphics[scale=0.60]{introduction/MappingStrategies-2_cropped.pdf}\centering
...
When working with \Rnaseq\ data, the typical next step after mapping the reads
to the reference is to quantify the expression of the feature\footnote{E.g.\ genes,
...
many potential initial sites. These reads are also called \emph{multireads}.
One early strategy to solve multireads is to discard them from later analyses;
another one is to attribute them to the most credible site based on the overall
distribution of the reads for a given sample. \mycite{Mortazavi2008}

Within this thesis, I use two popular tools,
...
(\htseq) performing
the feature quantification with a more stringent strategy. It discards all
\emph{ambiguous} reads (multireads or reads overlapping more than one gene) from
a \gls{SAM}/\gls{BAM} file to keep the \emph{unambiguous} ones only.
Then, it counts their
overlaps with the features of interest which are described in a given gene
model\footnote{Gene models are distributed as annotation file (usually either as
\gls{GTF} or \gls{GFF} format) to a specific reference (genome or transcriptome).}.
To quantify the expression of a given gene for example, it counts every read
that unambiguously overlaps with all its annotated exons\footnote{Indeed, \htseq\
defines a gene as the union of all its exons.}. Three modes fine-tune the overlap
definition (see \Cref{fig:htseqMode}). For this thesis, I used the
``\texttt{intersection non-empty}'' mode. This mode avoids discarding too many
reads due to a too tolerant annotation (\ie\ additional possible features can
be included to the annotation while they are not properly defined yet).

Initially, this tool was designed for \emph{differential gene
expression analysis} (see \cref{minisec:dgea}). As multireads are irrelevant for
those studies, including or excluding them from the downstream analysis was
insignificant \mycite{htseq}. Interestingly, many papers (\eg\ \mycite{Fonseca2014},
\mycite{errorsRNAquant}, \mycite{tophatStarwhatever}) have since showed that
the gene expression estimation by \htseq\ while underestimated is overall
well-correlated with other \gls{RNA} quantification (\eg\ microarrays, RT-qPCR).

Moreover, quantifications with \htseq\ are highly correlated with \cuffl\
quantifications for most of the genes (over 85\%) \mycite{tophatStarwhatever}.
...
it were counted for both genes, the extra reads from the differentially
expressed gene may cause the other gene to be wrongly
called differentially expressed, too. Another design choice made
with the downstream application of differential expression testing
in mind is to count fragments, not reads, in case of pairedend
data. This is because the two mates originating from the
...
\minisec{Differential gene expression analysis}\label{minisec:dgea}
