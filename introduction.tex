...
High-throughput protein identification and quantification use \ms\
as the primary method of choice since it allies a good dynamic range with high
sensitivity and specificity \mycite{Aebersold2003}, \mycite{Broshphd},
\mycite{Cox2011}. Indeed, a shift in the field has occurred on the last decade
from a largely technology-driven research to a mainstream analytical tool for
life sciences \mycite{AebersoldMann2016}.
...
relies on \gls{LC-MS/MS} \mycite{Cox2011}, \mycite{Zhang2013}. In fact, all
...
\mycite{Aebersold2016}: they are unbiased and free from hypothesis and as such
are great tools for global discovery study. Indeed, \gls{DDA} studies survey all
the proteome at once and prior knowledge is unrequired. The lately increased
...
\mycite{Cox2011}, \mycite{Zhang2014}. In the same manner, I will restrict myself
to the techniques used in the aforementioned datasets.
...
As recent developments have significantly improved proteome analysis from
\gls{FFPE} samples and since they are still evolving,
they may compare soon to fresh-frozen (\gls{FF}) samples \mycite{Feist2015}.
...
consideration is the cell lysis and then extraction approaches used on
the protein as they may interact and disrupt the later characterisation step and
thus require appropriate picking. In addition, the physical and chemical of
...
of proteomic coverage $[$\cite{Zhang2014,Bruce2013,Cox2011}$]$. Indeed,
the concentrations of peptides/proteins in a sample may saturate the \ms\
characterisation capacity: the very abundant proteins are easily detected and
quantified, while rarer proteins may be missed entirely without specific
precautions $[$\cite{Liu2009,Cappadona2012}$]$ or without unbearably
increasing the analysis time per sample \mycite{Nilsson2010}.
As the proteomic field unfortunately lacks amplification method, only targeted
depletion strategies can be used. Also, digesting the proteins into peptide
mixtures and then separating them in smaller fractions before subjection to \ms\
may help to increase the coverage-depth while keeping a reasonable analysis
time-frame $[$\cite{Aebersold2003,Cox2011,Zhang2013}$]$.

\subsubsection{Depletion of highly abundant proteins}

Regrettably, there are very few strategies to remove the highly abundant proteins.
\cite{Zhang2014} reports two main strategies useful for clinical samples such
as the plasma.

In one hand, as there are a very limited number of highly abundant proteins, they
can be specifically targeted: either by selective precipitation or by (very
expensive) protocols using antibody arrays designed against the highest abundant
proteins.

In the other hand, there is a strategy aiming to equalise the proteome to analyse.
It may be based on combinatorial ligand libraries involving bead-supported
ligands (on the model of the \Rnaseq\ protocol) or on specific proteases.
The first method uses bead-conjugated hexapeptide ligands to capture
approximative equimolar amounts of proteins. The very abundant proteins, after
binding till saturation, will be unable to attach to any available remaining
ligand, and so, will be washed off. While this strategy is 20-fold cheaper than
the previous depletion one, there are two main drawbacks.
First, it presupposes that every protein has a corresponding hexapeptide ligand
into which to bind. Secondly, it also skews the relative proportions of proteins
that will be analyse. This second downside may also be true for the second method
to achieve equalisation of the proteomes. \cite{MichaelMentenDepletion} uses
selective digestion of the most abundant proteins into peptides while the bulk
of the other proteins are left undigested until the remains of the most abundant
proteins are filtered out. In general, the equalisation of the proteome improves
the characterisation (identification and quantification) of the scarcer proteins
\mycite{Zhang2014}.
...
\subsection{Characterisation (and quantification) through fragmentation profiles}

Mass spectrometry


Also, the use of two mass spectrometers in tandem (\gls{MS/MS} reduces
significantly the number of missed proteins \mycite{LCMSflaws}.

\TK{Bioinformatic strategies}
