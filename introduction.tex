...
The \glspl{rRNA} are the most abundant type of \gls{RNA} in any cell. In fact,
even though they are a very small part of the genome\footnote{For example,
...
the total population \mycite{biochbook}. Although there are interests to
study \glspl{rRNA}, most studies research  \mRNAs\ (which constitute about
3 to 5\% of the whole \gls{RNA} population) \mycite{molBiolCell} and or other
more scarce (as they are less numerous) \glspl{RNA}.
...
of interest. There are typically three strategies: either polyA-selection,
ribodepletion and (more complex) targeted amplification.  While these
...
tail at the 3' end of the \mRNAs\ (and a few other \gls{RNA} kinds,
\eg\ \glspl{lncRNA} \mycite{polyAlncRNA}) added
...
This protocol is probably the most widespread one and a dataset produced
following this protocol is known as a \emph{polyA-selected} dataset.

\subminisec{Ribodepletion}
This strategy is preferred for the study of any \gls{ncRNA} or when researching
the interaction of \mRNAs\ with other \glspl{RNA}. \TK{add example.} This
...
\Cref{subsec:castlepresentation}). Hence, while the dataset they have created is
defined as a total \gls{RNA} dataset, the protocol they have used is closer to
the following one.

\subminisec{Targeted amplification}
Targeted amplifications rely on primers that would be designed to target (or
avoid as for \cite{castleData}) specific sequence motifs of the genome. This
approach is lesser used for \Rnaseq\ studies but quite frequent with other
types of sequencing-base assays for the \glspl{ncRNA}. And while the protocols
and workflows are based on the \Rnaseq\ one, often they are designed by their
own specific name, \eg\ \gls{miRNA-Seq}.
