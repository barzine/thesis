...
\soft{Cufflinks} share many common authors. As such, \cuffl\ is able to assemble
...
In both case, \cuffl\ infers the smaller set of transcripts (and their
isoforms) that can explain the complete set of observed fragments.

\cuffl\ infers the most credible isoforms that are expressed for each gene.
...
may be very limited \mycite{oneDominant}. \cuffl\ tries to distribute
the observed fragments (including the multireads) to the most
parsimonious\footnote{As a requirement to Occam's razor, which may be
a debatable strategy \mycite{OccamRazorAndBiology}} set of isoforms.
Many information are integrated for this
purpose. Typical information include the overall reads (or fragments)
distribution, the reads (or fragments) spanning over splice-junctions,
the ones mapping unambiguously to only
one of the possible isoforms. When available, paired-end fragments are critical:
as they cover longer regions, the probability that they span multiple adjacent
exons is increased which helps to resolve the possible structure of the isoforms.




Finally, to compute the gene expression level, \cuffl\ aggregates \latin{per}
...
\htseq\ provides
three modes to fine-tune the overlap definition (see \Cref{fig:htseqMode}).
...
(\ie\ the annotation itself presents many overlapping definitions for a given pair
of feature and chromosome region).
...
\mycite{tophatStarwhatever}. Indeed, \htseq\ provides \emph{raw counts} that
require normalisation to avoid a few statistical biases. These are mostly related
to sampling issues.
...
Longer the genes and more event to pick it.
