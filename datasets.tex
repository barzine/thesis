...
or  only upon requests \citep{PandeyData})
or by third-parties \citep{BioGPS1}, \citep{EBIgxa},
\citep{Harmonizome} --- I only used data reprocessed from raw files.

The original quantifications  are either directly
available \citep{Uhlen2015} or only upon request \citep{PandeyData}
and the normalised values are often the products
of different methodologies, e.g.\ \citep{GTExTranscript} and \citep{Krupp2012}.

Intuitively, we can appraise that different processing protocols produce
different results.
