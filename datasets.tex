...
\eg\ \dataset{\Gtex} data \citep{GTExTranscript} and \dataset{Castle}
...
In this thesis, I present results computed on the quantification of these
5 datasets which have been processed through the same identical pipeline.

\NB As the \Gtex\ data is involved in many project within the \EBI\
...
tremendous task and provided me with quantification data mapped to the \hg{38}
human genome reference and \ens{76} annotation.
...
needed for the \dataset{Casle} files. Indeed these files are using an older
...
advantages of the capacities provided by computer clusters. Hence, my processing
workflow improved without any real change as regards for the steps or
third-parties software tools used.
...
all the reads to be trimmed to the same length. \irap\ will optimise the purity
length-balance to avoid more errors or biases due to smaller reads
\citep{Trimwisely} by trimming at most 15\% of the original length while discarding
more reads if necessary to maximise the length.
