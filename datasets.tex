...
Every of the datasets, with which I worked, are fitting three main criteria.
They comprise human normal samples from at least three kind of tissues.
They are high-throughput; either sequenced with \Rnaseq\ for the transcriptomic
studies or analysed with Mass spectrometry for the proteomic ones.
Finally, the \emph{raw} data is available and reusable.

For the proteome, there are two published datasets that fit these
criteria. Due to Dr James Wright and Dr Jyoti Choudary tenacity a third dataset
was also added to the compendium.

There are a few more studies that I would have like to use on the
...
There are available quantifications for the different
datasets released either by the original authors or third parts: \citep{Krupp2012},
\citep{VTpaper}, \citep{BioGPS1}, \citep{Uhlen2015}, \citep{GTExTranscript},
\citep{PandeyData}, \citep{KusterData}. These quantifications can be partial or
complete; raw or normalised; available freely or only upon request.

For this study, I have all the datasets reprocessed from the raw files.
In fact, along my work, I noticed many potential bias sources that can highly impact
\Rnaseq results. Many of them have since then been reported in the literature.
Thus, annotations \citep{annotationDiff}, contamination \citep{contaminationRNAseq}
quality controls  and
mapping and quantifications pipelines \citep{Fonseca2014}
have great effects on the final raw quantification.

The normalisation of the quantified data has also great effect \citep{Dillies2013},
\citep{normalisation2}.
