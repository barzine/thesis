...
\TK{ajouter laius que tout a été fait par moi sous la supervision d'Alvis sauf
mention contraire}
...
These data raw files were downloaded from ProteomicsDB (identifier
\Proteomicsdb{PRDB000012}).

\section{Constant processing pipelines}

Despite available quantifications for most of the datasets, for many reasons
I only used data reprocessed from raw files.

The original authors often released
...
I could have used for most of the analyses quantification data downloaded
from EBI Gene Expression Atlas, if the data was already available at the time that
I started my study.}.
...
Additionally, the normalisation method of the quantified data also
greatly impacts the final expression values
\citep{Dillies2013}, \citep{normalisation2}.

Moreover, the available normalised \Rnaseq\ values of the datasets I use
are the products of different methodologies, \eg\ \citep{GTExTranscript}
and \citep{Krupp2012}.
...
tremendous task and provided me with quantification data (as raw and normalised
counts).
...
Indeed, as I had access to the different datasets through an extended period of
times, I built, annotated (from ENSEMBL 73 to ENSEMBL 75 -- all based on
the Human genome GRCh37) and then quantified, normalised and analysed a subset of
the datasets several times.
Those results are congruent with the latest ones.

In the early stages of my research, I was running each of the different steps
sequentially and manually. While the \EBI\ cluster greatly facilitated the
processing of the numerous files ,    the many integrity checks of the intermediate
files  % parler des customs scripts que j'ai ecrit pour faire les trucs plus rapidement
...
\begin{tabular}[c]{@{}c@{}}Average number\\of biologic samples\\ per tissues\end{tabular} \\
\midrule
Castle        & 11    & 11    & 11    & 58    & 10 (mixture)  \\
Brawand       & 18    & 21    & 23    & 111   & 2.8           \\
Ill. Body Map & 16    & 36    & 48    & 1,004  & 1             \\
Uhlén         & 122   & 200   & 400   & 1,851  & 3.81          \\
\textit{\color{darkgray}\Gtex\ v.1.4}  &
    \textit{\color{darkgray}3,276}  & \textit{\color{darkgray}3,276}  &
    \textit{\color{darkgray}6,552}  &
    \textit{\color{darkgray}>800,000}  & \textit{\color{darkgray}60.67}\\
\bottomrule
\end{tabular}
\end{table}
