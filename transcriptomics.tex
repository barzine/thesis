...
in a second part,
I base my analyses to appraise the congruence of gene expression profiles
across multiple transcriptome studies (described in \Cref{ch:datasets}).
...
All mitochondrial genes have been filtered out as specified in \Cref{subsec:mito}.
...
for \setOne\ (four shared tissues across the five studies) and \setTwo\
(twenty-three shared tissues between \uhlen\ \eta\ and \gtex\ studies).
...
Thus, it highlights a greater biological similarity of the \treps\
...
when I increase the expression threshold that genes have to comply to be
...
(except for \kidney).
In any case, \setTwo{}'s results should be interpreted more carefully as
...
expression profile across many tissues as the distribution of the coefficients
of variation across \uhlen\ \etal\ and \gtex\ studies are very alike
and \Cref{fig:noMitoNoRep23T} has already established that
...
\begin{figure}[!ht]
    ...
    from the heatmap as their expression profile is on average similar across
    tissues and studies.
...
As~\cite{DESeq2} noted the most common approach for detecting
...
\citet{Uhlen2015} uses cut-offs of $2$ and $5$
...
Much interlaboratory or interstudy research in the literature use Hampel's test,
\eg~\citet{LinsingerHampel,Lewczuk2006-wq,Rocke1983-qa,Apfalter1999-ca}.
In fact, Hampel's test is a robust method
...
I have relied on three methods to pick them.
For one of the method,
I have extracted from an existent resource
\WebCi{TiGER}{http://bioinfo.wilmer.jhu.edu/tiger/}{tiger},
the list of \gls{TS} genes that I have then updated to the more recent annotation
...
through the gene expression fold change (FC).
