...
is the dominant criterion for the clustering of the samples.
...
\eg\ \fallopian\ and \Ovary\ samples, \salivary\
with \Esophagus\ or \Stomach\ samples.
...
Once established that \Rnaseq\ allows distinguishing a common biological source
...
Similar variation patterns in the \pcgs\ across the tissues within each dataset
...
\Cref{fig:HistCV4T} reveals that the coefficients of variation are bimodally
distributed in addition to sharing very similar profiles.
Observe that there are two pics: one at $0.5$ and a second one at $2$ for
the \cvs.
After more extensive investigation, it appears that
the \pcgs\ with a coefficient of variation equal to or greater
than $1.5$ have two kind of distinct profiles:
\begin{itemize}[topsep=0pt,nosep]
...
The expression profiles of the genes with a \cv\ lesser than or equal to $0.5$
are similar to the expression profiles in \Cref{fig:distribPlot}, set apart
the left side which is truncated (due to the $1$ \FPKM\ cut-off).
...
The coefficients of variation of the \pcgs\ of the four tissues present
...
\begin{figure}[htpb]
    ...
    {transcriptomics/Heatmap_25p_mostCoeffVariablesgenes.pdf}\centering
    \caption[ee]{\label{fig:ee}\textbf{ee}ee}
\end{figure}


\begin{figure}[htpb]
    \includegraphics[scale=0.90]%
    {transcriptomics/Reverse_heatmap_25p_mostCoeffVariablesgenes.pdf}\centering
    \caption[ff]{\label{fig:ff}\textbf{ff}ff}
\end{figure}

\NB\ The same analysis involving the variance in lieu of
the coefficient of variation is also unable to give more insights
