...
\begin{itemize}[topsep=0pt,nosep]
    \item\hspace{-1.8mm}\fullcite{EBIgxa}
...
\section{Meta-analyses working sets}
\vspace{-2mm}
Through this chapter meta-analyses, I use two working sets
...
\begin{figure}[!htpb]
    ...
    regardless of their study sources (x-axis colourbar).}
\end{figure}
...
for both of the working datasets \setOne\ (4-tissue set) and \setTwo\ (23-tissue set).
...
working set (\tissue{Salivary gland} --- Pearson correlation: $r$=0.2)} and the
...
similarity}
\sectionmark{Possible driving force for a closer intratissue similarity}
...
For this purpose, I have tested the highest, the most variable
and the tissue specific genes
as potential underlying forces of the strong correlations.\mybr\

\subsection{Highest expressed genes}\label{subsec:mrnaHighExp}
...
for each possible corresponding tissue pair
of both working datasets \setOne\ and \setTwo.\mybr\

For \setOne\ (see \Cref{fig:CorHighExp4T}),
the highest thresholds (expression cut-offs) give the lowest correlations.
Conversely, the lowest cut-off (1 \FPKM) give the highest correlations.
In fact, as the expression cut-off increases,
both the number of genes involved in the correlation computation
and same-tissue \trep\ correlation coefficients decrease
(except rare exceptions presented in \Cref{subsec:mrnaHighExp-app}).

For \setTwo\ (see \Cref{fig:CorHighExp23T}),
...
As the meta-analyses based on Spearman correlation present higher
correlation coefficients than with Pearson method (see \Cref{fig:distribCorr}),
I have also explored the relation of the expression levels of the genes
with the levels of Spearman correlation
(see \Cref{sec:overlapHighExp} and \Cref{fig:overlapConcept}).
The general trend for \setOne\ follows a random one (see \Cref{fig:highExpress4T}).
Indeed, only the top highest \pcgs\ in each tissue (about 10--20)
are shared across all the five studies included in \setOne.
All the other \pcg\ ranks are indistinguishable from any random ordering.
\setTwo\ present a greater number of highly expressed \pcgs\ commonly shared
by the two studies included in \setTwo\
but this trend may solely be
the result of the smaller number of involved datasets
(see \Cref{fig:highExpress4T}).\mybr\

Together, these results and the ones presented in \Cref{subsec:mrnaHighExp-app}
indicate that the highest expressed genes are unable to explain
...
the \emph{restricted range}.~\mycite{CorrelationImpactingFactors}\mybr\
...
The overlap of the most variable genes between the five datasets
is only partial (see further \Cref{fig:cvEvol5DF}).\mybr\
...
then their group has to present higher correlations than the second set.\mybr\
...
The remaining genes constitute the second group $S_{remaining}$\footnote{%
$S_{remaining} = $ \setOne{}$- S_{most~Variable}$}.
For both of these gene groups, $S_{most~Variable}$ and $S_{remaining}$,
I calculate the correlation coefficients for all possible tissue couples.\mybr\

\begin{figure}[!htpb]
    \includegraphics[scale=0.7]%
    {transcriptomics/TransPearsonDistributionIdenticalDifferentHighestCVgenesA.pdf}%
    \centering
    \vspace{-0.2in}
...
original study (compare \Cref{fig:heatmapMost25pVariable,fig:ReverseheatmapMost25pVariable}).
...
I have generalised this analysis to the whole set of \setOne\ and \setTwo\ genes.
I have studied the course of the intersection size of the commonly shared genes
across the five studies of \setOne\ and the two studies of \setTwo\
when the number of considered genes increases.
\Cref{fig:overlapConcept} illustrates my general approach
and \Cref{subsec:checkMostVar} details the results supporting that
the first quarter of the most variable genes is an acceptable cut-off.

In conclusion, while many other gene groups may contribute
(\eg\ tissue-specific genes),
the most variable genes have a definitive and significant influence
on the strong and biologically meaningful correlations.\mybr\


\subsection{Genes with tissue-specific (TS) expression}\label{sub:TisSpeGene}
\vspace{-.25in}
Another good candidate to drive high correlations
between identical tissue interstudy \treps\
...
seems a reasonable approach to explain the high correlations.\mybr\

\subminisec{Tissue specificity definition}
...
\subminisec{TS genes characterisation approaches used in this thesis}
...
\begin{figure}[!htbp]
    \includegraphics[scale=0.7]{transcriptomics/mostSpe23TP1.pdf}\centering
    \vspace{-0.2in}
...
Thus, the \gls{TS} genes, identified separately in each dataset,
...
\section{Stability~of~gene~expression~profiles~across~studies}
Together the results from the previous sections indicate that
different categories of genes contribute to the strong tissue correlations.
In other words, many genes categories (if not all of them) have
an equivalent (\ie\ stable) expression profile
across studies for the same tissues.

I use the Uhlén \etal\ classification as a first approach to extend
the number of identified gene categories
that show interstudy expression stability.
...
The following classification considers the breadth,
...
I have constructed two meta-analyses datasets of \pcgs.
...
I have considered different attributes and groups of genes.\mybr\

The most variable and most specific genes have a real contribution
on the interstudy tissue strong correlations.
...
Conversely, the top highest expressed genes show little (if any) impact
on the correlations.\mybr\

Since correlations based on the whole datasets are among the highest observed ones,
other gene categories also have to contribute
and thus present interstudy expression stability.
To identify new contributors,
I have explored the congruence of the genes categories across the studies
that follow a classification
...
$r<0.65$.}.\mybr\
%March2015
\item\hspace{-1mm}\fullcite{Danielsson2015-cn}, cover three different tissues
...
%June2015 -Santos
\item\hspace{-1mm}\fullcite{Santos2015-rj}, focus on the congruence of gene---tissue association
...
despite any noise created by batch effect or technical variations.
Most variable and tissue-specific (\gls{TS}) genes are
two gene categories that drive independently
robust interstudy tissue expression correlations.

Although highest expressed genes fails to show any contribution
to the tissue correlations,
more detailed analyses highlight that
the expression profile of the genes is globally stable across studies
regardless of the gene category.
To assist further research,
I provide extensive curated and consolidated gene sets
for the different categories reviewed in the above analyses.\mybr\
...
However, each single distinct gene attribute has a reduced effect on the correlations.
Besides, the equivalent tissue expression profiles across the studies
...
use normalisation methods explicitly developed for \gls{DGEA}  %\glsxtrfull{DGEA}
