...
\begin{equation*}
        \setOneMath: \mathcal{D}_{\text{Trans}_1} x
                     \mathcal{G}_{\text{protein coding}_1} x
                     \mathcal{T}_1
\end{equation*}
and
\begin{equation*}
        \setTwoMath: \mathcal{D}_{\text{Trans}_2} x
                     \mathcal{G}_{\text{protein coding}_2} x
                     \mathcal{T}_2
\end{equation*}
...
when a study comprises technical and biological replicates for a given tissue
(\eg\ \brawand\ et al., \uhlen\ et al.\ and \gtex),
the gene expression is aggregated into a single virtual reference, \ie\ \trep,
such as there is only one measure per gene for each tissue in each study.
...
\begin{figure}[!htpb]
    ...
    regardless of their study sources (colourbar on top following the x-axis).}
\end{figure}
...
for both of the combined datasets \setOne\ (4-tissue set) and \setTwo\ (23-tissue set).
...
\section{Tissue specific genes: a driving force of the high interstudy
correlations}\label{sub:TisSpeGene}
...
correlations rather than intrastudy ones.\mybr\

After excluding possible sources of spurious Pearson correlation
due to null values by excluding the \emph{undefined}\footnote{I.e.\
...
I have checked, and then, discarded the highest expressed genes
as a possible driving force of the high correlations due to skewed distributions.

The high correlations are the results of the overall similarity
of genes expression patterns across tissues and studies.
...
\subsection{Use of prior knowledge: TiGER database}\label{subsub:Tiger}
...
\subsection{Fold change method}\label{subsub:TisSpeGeneMethodPerso}
...
\subsection{Hampel's test: detection of \emph{atypical} expression}\label{subsub:Hampel}
...
their expression is relatively invariant across tissues (\Cref{fig:HistCV4T,%
fig:HistCV23T}).
