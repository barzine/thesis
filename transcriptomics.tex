...
\begin{figure}[!htpb]
    ...
    is associated to one of the study, and
    the colours on the side bar following the y-axis
...
fail to show any significant correlation (see \Cref{subsec:mrnaHighExp-app}).\mybr\
...
\begin{equation}
    ...
    \FCm_{g,t}=\frac{x_{(g,t,d)}}{x_{\text{max}(g,\mathcal{T},d)}}
\end{equation}
where:\quad\begin{eqlist}[\setlength{\itemsep}{0em}%
        ...
        \item[\textbullet\ $x_{\text{max}(g,\mathcal{T},d)}$] $≥ x_{(g,t,d)}$
            $\forall t \in \mathcal{T}$,
            and $\forall g$,
            $\exists t_m$ $x_{(g,t_m,d)}= x_{\text{max}(g,\mathcal{T},d)}$
\end{eqlist}
