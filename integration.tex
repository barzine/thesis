...
%between the proteins ($\log_2(\text{PPKM}+1)$)
%and the \mRNAs\ ($\log_2(\text{FPKM}+1)$)
...
\vspace{-1.5mm}
As the results are sensibly similar
regardless of the protein quantification method or
the transcriptome source (either \uhlen\ \etal\ or \gtex\ study),
hereafter, I mostly present the results for the common set of fifteen tissues
between \uhlen\ \etal\ and \pandey\ \etal\
and their matching pairs of \mRNAs/proteins (quantified with the \PPKM\ method)
as this combination presents the most extensive set of tissues and genes to study.
...
%I mainly focus on two approaches.
...
\section{Influence of the expression breadth}
...
\begin{figure}[!htbp]
    \includegraphics[scale=0.75]{integration/uniqueFeatureQ3T15.pdf}\centering
    \caption[blabla]{\label{fig:UniqueFeatureQ3T15}\textbf{<++>}<++>}
\end{figure}

\begin{figure}[!htpb]
    \includegraphics[scale=0.75]{integration/coloredSharedbreadthProtQ3--15.pdf}\centering
    \caption[Protein expression breadth coloured according
    to its corresponding mRNA one]{\label{fig:SharedBreadthProtQ3}%
    \textbf{Protein expression breadth coloured according to
    its corresponding mRNA one.}  }
\end{figure}
...
%\section{Uniqueness of a protein can not be deduced by the uniqueness of a mRNA}
