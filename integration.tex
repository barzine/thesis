...
(see \Cref{sec:CorrMore}) to compare them.
...
\begin{figure}[!htbp]
    ...
    pairs.} Depending on the protein quantification method,
    there are two types of distribution ranges for the Pearson correlations.
    Top3 quantification method provides lower correlation ($\text{mean} \~{} 0.17$).
    ...
    The Spearman correlation median is above $0.55$.
    ...
    (all p-values <0.03 (Welch t-test) --- see \Cref{tab:pvalueCorrSP}).
...
Spearman correlations between these independent transcriptomic and proteomic \treps\
are equivalent to same-study-sampled transcriptomics and proteomics
described in the literature for cells.
As shown in \Cref{fig:TestSig},
the median Spearman correlations are above $0.5$
...
(Top3~\mycite{Silva-Top3} or \PPKM{}~\Cref{eq:PPKM} ---~\vref{eq:PPKM}).
Besides, for the \PPKM\ quantification (\Cref{sec:NewQuantProt})
the average Pearson correlation per tissue is also above $0.5$
$[$min:~$0.45$~(\Oesophagus)\;; max:~$0.67$~(\Liver)$]$
and close to the literature.

%When visualised: each paired couples: scatplot
...
\TK{citation for Welch t-test}\mycite{Welch1947-rv,Welch1951-sj} (and maybe \mycite{Ruxton2006-yy}).
