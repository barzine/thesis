...
in \Cref{fig:heatmapPandeyTissuePairs,fig:heatmapUhlenTissuePairs05}.
...
\Cref{app:geneCentric} contains complementary results
    ...
    where:
     \quad\begin{eqlist}[\setlength{\itemsep}{0em}%
         ...
         \item[\textbullet\ $\mathcal{C_a}$] is a decreasing sequence of $\mathcal{c_{ag}}$,
             ...
             %with $f:(\mathcal{a,G})\rightarrow \mathcal{C_{aG}}$
...
from $r= -0.77$ (for \gene{ZNF770}) to $r=1$ (for several genes).
...
difference in expression scale or another reason,
the comparison of the monotony of the variables by Spearman correlation
is favoured over the comparison of their linearity by Pearson correlation.
...
and enables reaching the same \emph{Pearson correlation} range
...
than highest expressed \mRNAs{} do.
...
Hence, both the \mRNA\ and protein stability of each pair have
a direct impact on their quantification,
and, therefore, on their correlation too.
...
\citet{Franks2017-bp} also show that genes from a same set of \gls{go} terms
...
since the proteomics and transcriptomics are sampled form the same sources
...
A possible approach is the annotation standardisation.
Optimisation or new algorithmic strategies are another one
