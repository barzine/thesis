...
following joint transcriptome/proteome studies
...
and confirmed that the true Pearson correlations are
...
This study measures \mRNA\ and corresponding protein copies per cell
with great care;
strand-specific \Rnaseq\ libraries (see \Crefu{strand-specific}) were
...
Indeed, it was not until 2014 that the first studies
to extensively investigate and quantify
the non-diseased human proteome at tissue level
were published~\mycite{PandeyData,KusterData}.
These are the MS studies that
...
While all the available data sets have been collected from different individuals,
and then prepared and characterised (\ie\ sequenced for the transcriptome samples
or ionised/fragmented for the proteome)
in independent laboratories,
the idea is to see how the findings at cells levels are `translatable' at tissue
level.
...
We attend to found out if the lesser correlations reported at cell levels would
