...
Furthermore, while this thesis redaction,
\citet{Franks2017-bp} and
\citet{SciRep2016}
published closely related studies to my work,
particularly as they are using data that I have included in my analyses
(\eg\ \gtex\ transcriptome data or \pandey\ proteome data),
I will describe their relevant findings and discuss them jointly with my own.
...
whereas good correlations should be observed (or not).
...
\citet{Vogel2010-ux} argue that two-thirds of the human protein abundance variation
can be explained by \mRNA\ estimations.
...
are (still) perceived as the main probable sources
...
Fascinatingly, this observation is unrestricted to human or
even mammals~\mycite{Maier2009-pb,Maier2011-tz,Palmblad2013-ji,Freiberg2016-fu}.
Many other cell studies have also lacked to show high correlation
...
(in the original publication version)
rather modest correlations ($r<0.61$)
for their encompassing and quite cleverly designed experiment.
Their model was comprising absolute \mRNAs\ and proteins levels,
and, for more than 5,000 genes,
their half-lives, and the transcription and translation rates.
...
while only about 40\% of protein variations are explained by \mRNAs\ levels.

This is probably why
following studies jointly done on the transcriptome
and the proteome
have mostly shifted their focus
from trying to link their actual expression
to comparing qualitative attributes
such as the presence or absence of given proteins and related \mRNAs\
in a specific condition or tissue~\mycite{Santos2015-rj,Freiberg2016-fu,Uhlen2015}
or if they are differential expressed
in the same set of conditions~\mycite{Varemo2015-uk}.
A few recent studies however are reporting improved correlations.
\citet{Li2014-ai} propose a rescaling method
to correct the general underestimation of protein abundance by label-free \ms.
Although their approach allows to improve correlations observed
...
remain limited ($r≤0.45$).
In the meanwhile,
the authors of the original paper~\mycite{schwanhausserglobal:2011}
realised that a computational mistake happen at their scaling step
and that the true Pearson correlations are about $0.64$~\mycite{Schwanhausser2013-et}.


\citet{Marguerat2012-sn} report higher \mRNAs/proteins Pearson correlations
(up to )
in yeast which are based on the $\log_{2}$-scales of
their respective copies per cell.
They have measured the \mRNAs\ and the proteins with great care;
their \Rnaseq\ libraries,
which were prepared with strand-specific protocols (see \Crefu{strand-specific}),
are recalibrated based on 49 \mRNAs\
for which absolute data was acquired from cell extracts spiked with
known quantities of external \RNA\ controls;
their protein samples were also recalibrated:


Although these correlations are around
$0.4$ (Pearson correlation coefficient).


There are different ways to integrate transcriptomic and proteomic data\label{sec:IntegrationPossibleApproaches}.
...
High-throughput technologies for proteomics are still lacking
when compared to sequencing technology.
Mass spectrometry (\ms) is however one of the most accurate ways to quantify
proteins abundance. However, often the top most abundant proteins can
...
As recently, there were two big releases of Proteomics studies of normal human
tissues, we attend to found out if the lesser correlations reported at cell levels would
...
\section{Introduction}\label{sec:IntegrationIntro}
