...
\externaldocument{app/protNoTrans}
\externaldocument{app/IntScatt}
%%%%%%
...
\fixme{see bottom of page 2 in the paper}
...
\begin{figure}%[!htbp]
\includegraphics[scale=0.65]{expressed.png}\centering
...
\begin{figure}%[!htbp]
    \includegraphics[scale=0.90]{integration/KusterPandey1_2FQM.png}\centering
    ...
    \dataset{Pandey \etal} dataset than the \dataset{Kuster \etal} dataset. We
    can notice that while \tissue{Testis} is f !!!! 
    }
\end{figure}
...
\fixme{add: `Indeed the original publication was reporting XX uniquely detected
proteins.'}
...
proteomic and the transcriptomic dataset is from $0.66$ for the \tissue{Liver}
to 0.47 for the \tissue{Oesophagus}. \rough{If I exclude the lowly correlated genes
between \dataset{GTEX} and \dataset{Uhlén \etal}, then while the \tissue{Liver}
is still the tissue with the best correlation between proteins and \mRNAs\ and
the \tissue{Oesophagus} has the worst correlation,
the range of Pearson correlation is now from $0.66$ to $0.46$
(instead of $0.66$ to $0.47)$. Besides, If I exclude the \mRNAs\ that are expressed
below $1$ \gls{FPKM} (and their corresponding proteins), the Pearson correlation
coefficients for each tissue are substantially identical.}

\begin{figure}%[!htbp]
    \includegraphics[scale=0.6]{integration/Kidney_scattplot.pdf}\centering
...
In~\cref{fig:ScatKid}, we can see on %######
...
\begin{figure}
    ...
    coefficients for same tissues pairs (e.g.\ proteomic\textsubscript{\tissue{Liver}}
    $\sim$ transcriptomic\textsubscript{\tissue{Liver}},
    proteomic\textsubscript{\tissue{Heart}} $\sim$
    transcriptomic\textsubscript{\tissue{Heart}}, etc.).
    The right the correlation of all the possible random
    pair of tissues (e.g.\ proteomic \tissue{Lung} $\sim$ transcriptomic
    \tissue{Liver}, proteomic \tissue{Heart} $\sim$ transcriptomic \tissue{Kidney},
...
\begin{figure}
    \includegraphics[scale=0.85]{integration/breadthTrans.png}\centering
