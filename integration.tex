...
\taset{Kuster}subsection{Transcriptome}
...
\glspl{FPKM} output of HTseq. Overall, either if we use \dataset{Uhlén et al} or
...
\begin{figure}%[!htbp]
    \includegraphics[scale=0.55]{integration/PandeyGtexUhlen_tissuesVenn.png}
    \centering
...
\begin{figure}%[!htbp]
    \includegraphics[scale=0.55]{integration/KusterGtexUhlen_tissuesVenn.png}
    \centering
...
\begin{figure}%[!htbp]
    ...
    \subcap{Highlighted in red are the proteins that are found only in that
...
\subsubsection{Similar shape of density between proteomic and transcriptomic
...
\begin{figure}
    ...
    \subcap{Most of the proteins are only found in one tissue. The second largest group is
    consisting of the ubiquitous proteins.}
\end{figure}
...
However, when we apply a threshold, either 1 \gls{FPKM} (\ref{breadthTrans}|C) or
5 \glspl{FPKM} (\ref{breadthTrans}|D), to which the \mRNA\ expressions have to be
...
\begin{figure}
    ...
    \subcap{The distribution is given for several threshold. \textbf{A}| Every time a
    \mRNA\ is detected (> 0 \gls{FPKM}). \textbf{B}| Every time a \mRNA\ is
    detected at or above 1 \gls{FPKM} and \textbf{C}| 5 \glspl{FPKM}.}
\end{figure}
...
In Figure~\ref{barPlotunique}, we can see that \tissue{Testis} and \tissue{Liver}
...
After this first phase of \emph{manual} checks, 
